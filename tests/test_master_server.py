...
def _register_node(client, node_id="n1", **kwargs):
    ...
    return client.post("/api/nodes/register", json=payload)
...
class TestMasterServerTaskManagement:
    ...
    async def test_list_tasks(self, client, master_server):
        await _register_node(client)
        await client.post("/api/tasks/submit", json={"name": "task1", "mode": "data", "model_name": "m1"})
    ...
    async def test_get_task(self, client, master_server):
        await _register_node(client)
        submit_resp = await client.post("/api/tasks/submit", json={"name": "task1", "mode": "data", "model_name": "m1"})
    ...
    async def test_cancel_task(self, client, master_server):
        ...
        submit_resp = await client.post("/api/tasks/submit", json={"name": "task1", "mode": "data", "model_name": "m1"})
        task_id = submit_resp.json()["task_id"]
        resp = await client.post(f"/api/tasks/{task_id}/cancel", json={"reason": "user request"})
        assert resp.status_code == 200

    @pytest.mark.asyncio
    async def test_cancel_task_missing(self, client):
        resp = await client.post("/api/tasks/nonexistent/cancel", json={"reason": "test"})
        assert resp.status_code == 404
    ...
    async def test_cancel_completed_task(self, client, master_server):
        ...
        resp = await client.post(f"/api/tasks/{task_id}/cancel", json={"reason": "too late"})
    ...
    async def test_migrate_task(self, client, master_server):
        ...
        submit_resp = await client.post("/api/tasks/submit", json={"name": "task1", "mode": "data", "model_name": "m1"})
        task_id = submit_resp.json()["task_id"]
        resp = await client.post(f"/api/tasks/{task_id}/migrate")
        assert resp.status_code == 200

    @pytest.mark.asyncio
    async def test_migrate_task_missing(self, client):
        resp = await client.post("/api/tasks/nonexistent/migrate")
